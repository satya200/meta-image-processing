...
/*
 * Name: bmp_img_reduce_grey_level()
 * Desc: This is used Reduce greay level according to 
 *	 user input.
 * Parameter: No Parameter
 * Return : 0 = on Success
 *          < 0 = On Failure
...
int bmp_img_reduce_grey_level()
{
	...
	unsigned int greay_level = 0;

	printf("Please Enter greay scale image name to controle greay scale\n");
	scanf("%s",src_img_name);
	sprintf(img_name_src_path, "%s/%s",IMG_SEARCH_PATH, src_img_name);
	printf("Please enter greay_level:");
	getchar();
	scanf("%u",&greay_level);
	sprintf(img_name_dst_path, "%s/grey%u_%s", IMG_SEARCH_PATH, greay_level, src_img_name);
	printf("Source image:%s\nDest image:%s\n",img_name_src_path, img_name_dst_path);
	...
	strcpy(img_wr.new_image_name, img_name_dst_path);
        ...
        memcpy(img_wr.colorTable, img_rd.colorTable, sizeof(img_rd.colorTable));
        //memcpy(img_wr.buf, img_rd.buf, sizeof(img_rd.buf));
        img_wr.bitDepth = img_rd.bitDepth - 6;

        img_size = (img_rd.height * img_rd.width);
	buffer = (unsigned char *)malloc(img_size * sizeof(unsigned char));
	if (buffer == NULL) {
		printf("buffer null malloc return\n");
		return -1;
	}
	for (int i = 0; i < img_size; i++) {
